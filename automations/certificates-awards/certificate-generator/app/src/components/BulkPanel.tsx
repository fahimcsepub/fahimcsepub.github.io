  ...
  return (
    <section className="wide-card">
      ...
      >
        ...
        <span>Full category names, custom aliases, and short AE, RE, OA, or CCEA values are accepted.</span>
                    ...
                    {expandedId === row.id && (
                      <tr className="detail-row"><td colSpan={7}>
                        <div className="row-details">
                          ...
                          {row.record.awardCategory === 'research' && <><label>Article title<Input value={row.record.articleTitle} onChange={(event) => updateRecord(row.id, 'articleTitle', event.target.value)} /></label><label>Journal<Input value={row.record.journalName} onChange={(event) => updateRecord(row.id, 'journalName', event.target.value)} /></label><label>Journal recognition<Select value={row.record.q1Verified ? 'q1' : 'general'} onChange={(event) => updateRecord(row.id, 'q1Verified', event.target.value === 'q1')}><option value="general">General publication</option><option value="q1">Q1 — verified</option></Select></label></>}
                          ...
                          {row.record.awardCategory === 'coordination' && <label>Coordination period<Input value={row.record.coordinationPeriod} onChange={(event) => updateRecord(row.id, 'coordinationPeriod', event.target.value)} placeholder="e.g. Spring 2025 – Summer 2026" /></label>}
